import React from "react";
import { useNavigate } from "react-router-dom";

// Type definition for user data
type UserProfile = {
  name: string;
  role: string;
  email: string;
  phone: string;
  location: string;
  avatarUrl?: string;
};

interface ProfileSectionProps {
  user: UserProfile;
}

const ProfileSection: React.FC<ProfileSectionProps> = ({ user }) => {
  const navigate = useNavigate();

  return (
    <div className="bg-white shadow-lg rounded-2xl p-6 w-full md:w-1/3 flex flex-col items-center text-center animate-fade-in">
      <img
        src={user.avatarUrl || "https://via.placeholder.com/100"}
        alt="User"
        className="w-24 h-24 rounded-full object-cover mb-4 ring-4 ring-primary-light"
      />
      <h2 className="text-xl font-semibold text-dark-text">{user.name}</h2>
      <p className="text-gray-500 mb-4">{user.role}</p>
      <div className="text-sm text-gray-600 mb-6 space-y-1">
        <p>Email: {user.email}</p>
        <p>Phone: {user.phone}</p>
        <p>{user.location}</p>
      </div>
      <button
        className="bg-primary hover:bg-teal-600 text-white px-5 py-2 rounded-xl transition shadow-md"
        onClick={() => navigate('/dashboard/profile/edit')}
      >
        Edit Profile
      </button>
    </div>
  );
};

export default ProfileSection;
